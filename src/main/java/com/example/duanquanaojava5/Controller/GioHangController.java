...
public class GioHangController {
    ...
    public String addCard(Model model,
        ...
        if (ctsp == null) {
            // Nếu chi tiết sản phẩm không tồn tại, trả về trang 404
            return "redirect:/404";
        }

        List<GioHangChiTiet> gioHangChiTiets = gioHangChiTietService.findByGioHangAndCtsp(gioHang, ctsp);
        if (gioHangChiTiets.isEmpty()) {
            // Nếu không có chi tiết giỏ hàng cho chi tiết sản phẩm này, tạo một chi tiết giỏ hàng mới
            ...
            // Nếu đã có chi tiết giỏ hàng cho chi tiết sản phẩm này, cập nhật số lượng sản phẩm trong chi tiết giỏ hàng đó
            GioHangChiTiet gioHangChiTiet = gioHangChiTiets.get(0);
            gioHangChiTiet.setSoLuong(gioHangChiTiet.getSoLuong() + soLuong);
            gioHangChiTietService.save(gioHangChiTiet);
        }

        // Chuyển hướng đến trang chi tiết sản phẩm tương ứng với chi tiết sản phẩm được thêm vào giỏ hàng
        return "redirect:/detail/" + ctsp.getSp().getMaSanPham() + "/" + ctsp.getSz().getMaSize() + "/" + ctsp.getMauSac().getMaMauSac();
    }
